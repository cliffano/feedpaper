...
Data.prototype.initCache = function () {
  ...
  this.categoryList.forEach(function (category) {
    category.feeds.forEach(function (feed) {
      ...
      // self.fetchFeed(feed.url, function (err, articlesList) {
      //   if (err) {
      //     console.error('Unable to fetch feed: ' + feed.id, err);
      //   } else {
      //     console.log("Set feed: " + feed.url);
      //     self.cache.set(feed.url, articlesList, function (err, success) {
      //       if (err) {
      //         console.error('Unable to cache feed: ' + feed.id, err);
      //       }
      //     });
      //     articlesList.forEach(function (article) {
      //       self.fetchArticle(article.url, function (article) {
      //         console.log("Set article: " + article.url);
      //         console.log(article.url);
      //         if (article.url === undefined) {
      //           console.dir(article);
      //         }
      //         self.cache.set(article.url, article, function (err, success) {
      //           if (err) {
      //             console.error('Unable to cache url: ' + feed.id, err);
      //           }
      //         });
      //       });
      //     });
